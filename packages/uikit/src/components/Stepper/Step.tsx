...
const Connector = styled.div<StatusProps>`
  ...
  background-color: ${({ theme, status }) =>
    theme.colors[status === "past" || status === "current" ? "overlay" : "overlay"]};
`;
...
export const StepNumber = styled.div<StatusProps>`
  ...
  background-color: ${({ theme, status }) => theme.colors[status === "current" ? "secondary" : "secondary"]};
  border: 2px solid ${({ theme, status }) => (status === "past" ? theme.colors.invertedContrast : "invertedContrast")};
