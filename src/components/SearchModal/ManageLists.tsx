...
import { Button, Text, CheckmarkIcon, CogIcon, Input, Toggle, LinkExternal, useTooltip } from '@scads-io/uikit'
...
const RowWrapper = styled(Row)<{ active: boolean }>`
  background-color: ${({ active, theme }) => (active ? `${theme.colors.overlay}19` : 'transparent')};
  border: solid 1px;
  border-color: ${({ active, theme }) => (active ? 'rgba(165,161,150,0.84)' : theme.colors.tertiary)};
  transition: 200ms;
  align-items: center;
  padding: 1rem;
  border-radius: 20px;
...
const ListRow = memo(function ListRow({ listUrl }: { listUrl: string }) {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <div>
      <Text color='overlay' bold>{list && listVersionLabel(list.version)}</Text>
      <LinkExternal color='overlay' bold external href={`https://tokenlists.org/token-list?url=${listUrl}`}>
        {t('See')}
      </LinkExternal>
  ...
  return (
    <Wrapper>
      <AutoColumn gap='14px'>
        ...
        {addError ? (
          <Text bold color='failure' style={{ textOverflow: 'ellipsis', overflow: 'hidden' }}>
            {addError}
          </Text>
        ) : null}
      </AutoColumn>
