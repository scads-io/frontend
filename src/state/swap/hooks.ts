...
import {
  useCaratBuyAmount,
  useScadsBuyAmount,
  useCaratBuyBackAmount,
  useScadsBuyBackAmount,
  useCaratTotalSupply,
  useScadsLatestMintInfo,
  useLatestBoughtInfo,
  useMintedCaratAmount,
  useScadsLatestSellInfo,
  useLatestRewardAmount,
  useCompoundAddressExist,
} from '../../hooks/useCaratTrade'
...
// from the current swap inputs, compute the best trade and return it.
export function useCaratBuyInfo(
  tokenAmount,
  outputToken,
): {
  currencyBalances: { [field in Field]?: CurrencyAmount }
...
// from the current swap inputs, compute the best trade and return it.
export function useCaratBuyBackInfo(
  tokenAmount,
  outputToken,
): {
  currencyBalances: { [field in Field]?: CurrencyAmount }
...
export function useScadsLatestSellDate() {
  const sellDate = useScadsLatestSellInfo()
  return parseInt(sellDate?.lastestDaySold)
}
...
export function useUserLatestRewardAmount() {
  return useLatestRewardAmount()
}

export function useUserCompoundExist() {
  return useCompoundAddressExist()
}
